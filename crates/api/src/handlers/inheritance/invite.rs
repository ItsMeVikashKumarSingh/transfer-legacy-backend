...
pub async fn create_invite(
    ...
    let hmac = compute_hmac(config.server_hmac_secret.as_bytes(), token_data.as_bytes()).map_err(
        |_| ApiError::app_with_request_id(transfer_legacy_shared_types::AppError::Internal, &rid),
    )?;
    ...
    let policy = fetch_policy(&state.db, policy_id).await.map_err(|_| {
    ...
    let invite_url = format!(
        "{}/invite/claim?invite_id={}&token={}",
        config.app_url, invite_id, claim_token
    );
    ...
    let owner_name = if let Some(enc_name) = policy.enc_owner_name {
        if enc_name.len() >= 24 {
            ...
            let key = URL_SAFE_NO_PAD
                .decode(&config.server_aead_key_b64)
                .unwrap_or_default();
    ...
    if let Err(e) = state
        .notify(policy.owner_id, &payload.email, template)
        .await
    {
