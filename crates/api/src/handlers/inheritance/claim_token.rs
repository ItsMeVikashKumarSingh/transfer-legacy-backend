...
pub async fn consume_claim_token(
    ...
    let token_bytes = URL_SAFE_NO_PAD.decode(payload.claim_token).map_err(|_| {
    ...
    verify_hmac(
        config.server_hmac_secret.as_bytes(),
        data.as_bytes(),
        &token_bytes,
    )
    .map_err(|_| {
    ...
    update_policy_participants(
        &mut tx,
        invite.policy_id,
        serde_json::Value::Array(beneficiaries_json),
        serde_json::Value::Array(approvers_json),
    )
    .await
    .map_err(|_| {
        ApiError::app_with_request_id(transfer_legacy_shared_types::AppError::Internal, &rid)
