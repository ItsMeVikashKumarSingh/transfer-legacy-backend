...
use transfer_legacy_shared_types::models::vault::{
    CreateItemRequest, CreateItemResponse, ListItemsResponse, GetItemResponse, ItemSummary
};
use uuid::Uuid;
...
async fn get_auth_token(client: &mut CryptoClient, user_id: Uuid, password: &str) -> String {
    ...
    let reg_init_res: RegisterInitResponse = client.post_aead("/v1/auth/register/init", &reg_init_req).await;
    
    // 2. Registration Finish
    let reg_upload = crate::tests::test_utils::test_client_register_finish(reg_state, &reg_init_res.registration_response);
    ...
    let _: RegisterFinishResponse = client.post_aead("/v1/auth/register/finish", &reg_finish_req).await;
    ...
    let login_init_res: LoginInitResponse = client.post_aead("/v1/auth/login/init", &login_init_req).await;
    ...
    let login_finish_res: LoginFinishResponse = client.post_aead("/v1/auth/login/finish", &login_finish_req).await;
...
async fn test_vault_lifecycle() {
    ...
    // --- 2. List Vault Items ---
    let list_res: ListItemsResponse = client.post_aead("/v1/vault/items/list", &serde_json::json!({"user_id": user_id})).await;
    assert!(!list_res.items.is_empty(), "Vault list should not be empty");
    assert!(list_res.items.iter().any(|i| i.item_id == item_id), "Created item missing from list");

    // --- 3. Get Specific Item ---
    let fetched_item: GetItemResponse = client.post_aead(&format!("/v1/vault/items/{}", item_id), &serde_json::json!({"user_id": user_id, "item_id": item_id})).await;
