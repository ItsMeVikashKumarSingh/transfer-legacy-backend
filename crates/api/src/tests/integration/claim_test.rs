...
use transfer_legacy_shared_types::models::claim::{
    ClaimInitiateRequest, ClaimInitiateResponse, ClaimConfirmRequest, ClaimConfirmResponse
};
use uuid::Uuid;
...
async fn test_claim_lifecycle() {
    ...
    let password = "ClaimPassword123!";
    let _token = get_auth_token(&mut client, user_id, password).await;
    
    println!("Claim test updated to use shared models for {}", user_id);
}
