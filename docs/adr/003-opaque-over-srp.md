...
## Decision
Use OPAQUE (opaque-ke v4) with Ristretto255 + HKDF-SHA512 for registration and login. Server state for in-flight sessions is stored in Valkey/Redis with a short TTL.

## Consequences
- Server never stores or sees plaintext passwords.
- Registration and login require multi-step client-server exchange.
- Valkey/Redis becomes part of the auth flow for transient protocol state.
